...
def measure_shear_metadetect(res, *, s2n_cut, t_ratio_cut, ormask_cut,
    ...
    def _mask(data):
        ...
        mask1 = data['wmom_flags'] == 0
        mask2 = data['wmom_psf_flags'] == 0
        mask3 = data['wmom_obj_flags'] == 0
        #failure in the fitting of the object
        mask4 = data['wmom_band_flux_flags'] == 0
        mask5 = data['wmom_T_flags'] == 0
        mask6 = data['psfrec_flags'] == 0
        mask7 = data['wmom_s2n'] > s2n_cut
        mask8 = data['wmom_T_ratio'] > t_ratio_cut
        #size ratio between the object and the psf

        _cut_msk = mask1 & mask2 & mask3 & mask4 & mask5 & mask6 & mask7 & mask8
        ...
        assert np.any(_cut_msk), "No objects pass cuts"

        # if np.mean(_cut_msk) < 0.5:
        # print("The mask fraction is:", np.mean(_cut_msk))
...
def _run_sim_pair(args):
    ...
    num, backend, sim_func, sim_kwargs, start, seed, gal = args

    pobs = sim_func(gal=gal, g1=0.02, g2=0.0, seed=seed, **sim_kwargs)
    mobs = sim_func(gal=gal, g1=-0.02, g2=0.0, seed=seed, **sim_kwargs)
    ...
    if pres is None or mres is None:
        raise RuntimeError("metadetect failed")
...
def run_mdet_sims(gal,
    ...
    noise cancel c   : {c: f} +/- {csd: f} [1e-5, 3-sigma]""".format(
            ...
            assert False, "no sims ran successfully"
            return None, None
